...
namespace Ildasm
{
    ...
    sealed partial class Disassembler
    {
        ...
        readonly Universe universe = new Universe(UniverseOptions.EnableFunctionPointers);
        ...
        readonly Type typeofSystemRuntimeInteropServicesDllImportAttribute;
        readonly Type typeofSystemRuntimeInteropServicesMarshalAsAttribute;
        readonly Type typeofSystemRuntimeInteropServicesFieldOffsetAttribute;
        ...
        internal Disassembler(string inputFile, string outputFile, CompatLevel compat)
        {
            ...
            typeofSystemRuntimeInteropServicesDllImportAttribute = universe.Import(typeof(System.Runtime.InteropServices.DllImportAttribute));
            typeofSystemRuntimeInteropServicesMarshalAsAttribute = universe.Import(typeof(System.Runtime.InteropServices.MarshalAsAttribute));
            typeofSystemRuntimeInteropServicesFieldOffsetAttribute = universe.Import(typeof(System.Runtime.InteropServices.FieldOffsetAttribute));
            var raw = universe.OpenRawModule(inputFile);
        ...
        void WriteModules(LineWriter lw)
        {
            foreach (var module in assembly.GetModules(true))
            {
                ...
                {
                    lw.WriteLine(".file nometadata {0}", QuoteIdentifier(module.Name));
        ...
        void WriteMscorlibDirective(LineWriter lw)
        {
            ...
            if (obj != null && obj.BaseType == null && obj.IsClass)
            {
                lw.WriteLine();
        ...
        void WriteData(LineWriter lw)
        {
            ...
            Array.Sort(fixups, (x, y) => x.RVA.CompareTo(y.RVA));
            dataFields.Sort((x, y) => x.__FieldRVA.CompareTo(y.__FieldRVA));
            int prevRVA = 0;
            int prevLength = 0;
            for (int fieldPos = 0, fixupPos = 0; fieldPos < dataFields.Count || fixupPos < fixups.Length; )
            {
                int currRVA;
                int currLength;
                if (fixupPos < fixups.Length && (fieldPos == dataFields.Count || fixups[fixupPos].RVA < dataFields[fieldPos].__FieldRVA))
                {
                    currRVA = fixups[fixupPos].RVA;
                    currLength = fixups[fixupPos].Count * ((fixups[fixupPos].Type & COR_VTABLE_32BIT) != 0 ? 4 : 8);
                    fixupPos++;
                }
                else
                {
                    currRVA = dataFields[fieldPos].__FieldRVA;
                    currLength = GetTypeSize(dataFields[fieldPos].FieldType);
                    fieldPos++;
                }
                if (prevRVA != 0)
                {
                    WriteData(lw, prevRVA, Math.Min(prevLength, currRVA - prevRVA));
                }
                prevRVA = currRVA;
            ...
            if (prevRVA != 0)
            {
                WriteData(lw, prevRVA, prevLength);
            }
        }

        void WriteData(LineWriter lw, int rva, int length)
        {
            if (length == 0)
            {
                return;
            ...
            string name;
            int characteristics;
            module.__GetSectionInfo(rva, out name, out characteristics);
            if ((characteristics & IMAGE_SCN_MEM_WRITE) != 0 && !hasNonZero)
            {
                lw.WriteLine(".data {0}_{1:X8} = int8[{2}]", (characteristics & IMAGE_SCN_CNT_CODE) != 0 ? "cil I" : "D", rva, buf.Length);
            }
            else
            {
                lw.WriteLine(".data {0}_{1:X8} = bytearray (", (characteristics & IMAGE_SCN_CNT_CODE) != 0 ? "cil I" : "D", rva);
                lw.GoToColumn(17);
        ...
        void WriteCopyrightHeader(LineWriter lw)
        {
            ...
            {
                lw.WriteLine();
                lw.WriteLine("//  Microsoft (R) .NET Framework IL Disassembler.  Version 4.0.30319.17020");
                lw.WriteLine("//  Copyright (c) Microsoft Corporation.  All rights reserved.");
                lw.WriteLine();
                lw.WriteLine();
            }
        ...
        void WriteType(LineWriter lw, IKVM.Reflection.Type type)
        {
            ...
            foreach (var ca in type.__GetCustomAttributes(null, false))
            {
                lw.GoToColumn(level + 2);
            ...
            foreach (var field in type.__GetDeclaredFields())
            {
                lw.GoToColumn(level + 2);
                WriteField(lw, field);
            }
        ...
        void WriteEvent(LineWriter lw, EventInfo evt)
        {
            ...
            lw.GoToColumn(level);
            lw.WriteLine("{");
            foreach (var ca in evt.__GetCustomAttributes(null, false))
            {
                lw.GoToColumn(level + 2);
                WriteCustomAttribute(lw, ca);
            }
            foreach (var acc in evt.__GetMethods())
            {
                lw.GoToColumn(level + 2);
                ...
                lw.Write("::{0}(", GetMethodName(acc));
                WriteParameters(lw, acc.GetParameters());
                lw.WriteLine(")");
            }
            lw.GoToColumn(level);
            lw.WriteLine("}} // end of event {0}::{1}", evt.DeclaringType.__Name, evt.Name);
        }
        ...
        void WriteProperty(LineWriter lw, PropertyInfo prop)
        {
            ...
            lw.GoToColumn(level);
            lw.WriteLine("{");
            foreach (var ca in prop.__GetCustomAttributes(null, false))
            {
                lw.GoToColumn(level + 2);
                WriteCustomAttribute(lw, ca);
            }
            foreach (var acc in prop.GetAccessors(true))
            {
                lw.GoToColumn(level + 2);
        ...
        void WriteMethod(LineWriter lw, MethodBase method)
        {
            ...
            if ((returnParameter.Attributes & ParameterAttributes.HasFieldMarshal) != 0)
            {
                ...
                WriteMarshalAs(sb, level1 - 1, returnParameter.__GetCustomAttributes(null, false), false);
            ...
            foreach (var parameter in method.GetParameters())
            {
                ...
                {
                    WriteMarshalAs(lw, level2, parameter.__GetCustomAttributes(null, false), true);
                }
            ...
            foreach (var ca in FilterPseudo(method.__GetCustomAttributes(null, false)))
            {
                lw.GoToColumn(level0 + 2);
                WriteCustomAttribute(lw, ca);
            }
        ...
        {
            IList<CustomAttributeData> ca = FilterPseudo(parameter.__GetCustomAttributes(null, false));
        ...
        {
            CustomAttributeData ca = null;
            foreach (var ca1 in method.__GetCustomAttributes(null, false))
            {
                if (ca1.Constructor.DeclaringType == typeofSystemRuntimeInteropServicesDllImportAttribute)
                {
                    ca = ca1;
                    break;
                }
            ...
            if (ca.ConstructorArguments[0].Value != null)
            {
                lw.Write("\"{0}\"", ca.ConstructorArguments[0].Value);
            }
            ...
            bool lasterr = false;
            string bestfit = null;
            string charmaperror = null;
            foreach (var arg in ca.NamedArguments)
            {
                switch (arg.MemberInfo.Name)
                {
                    case "CallingConvention":
                        switch ((System.Runtime.InteropServices.CallingConvention)arg.TypedValue.Value)
                        {
                        ...
                        break;
                    case "EntryPoint":
                        entrypoint = (string)arg.TypedValue.Value;
                        break;
                    case "ExactSpelling":
                        exactspelling = (bool)arg.TypedValue.Value;
                        break;
                    case "CharSet":
                        switch ((System.Runtime.InteropServices.CharSet)arg.TypedValue.Value)
                        {
                        ...
                        break;
                    case "SetLastError":
                        lasterr = (bool)arg.TypedValue.Value;
                        break;
                    case "BestFitMapping":
                        bestfit = (bool)arg.TypedValue.Value ? "on" : "off";
                        break;
                    case "ThrowOnUnmappableChar":
                        charmaperror = (bool)arg.TypedValue.Value ? "on" : "off";
                        break;
                }
            }
            if (entrypoint != null && entrypoint != method.Name)
            {
                lw.Write(" as \"{0}\"", entrypoint);
            }
            if (exactspelling)
            {
                lw.Write(" nomangle");
            ...
            if (bestfit != null)
            {
                lw.Write(" bestfit:{0}", bestfit);
            }
            if (charmaperror != null)
            {
                lw.Write(" charmaperror:{0}", charmaperror);
            }
            lw.Write(") ");
        }

        IList<CustomAttributeData> FilterPseudo(IList<CustomAttributeData> list)
        {
            List<CustomAttributeData> newList = null;
            for (int i = 0; i < list.Count; i++)
            {
                if (list[i].Constructor.DeclaringType == typeofSystemRuntimeInteropServicesDllImportAttribute
                    || list[i].Constructor.DeclaringType == typeofSystemRuntimeInteropServicesMarshalAsAttribute
                    || list[i].Constructor.DeclaringType == typeofSystemRuntimeInteropServicesFieldOffsetAttribute)
                {
                    if (newList == null)
                    {
                        newList = new List<CustomAttributeData>();
                        for (int j = 0; j < i; j++)
                        {
                            newList.Add(list[j]);
                        }
        ...
        void WriteMarshalAs(LineWriter lw, int level, IList<CustomAttributeData> list, bool wrap)
        {
            var sb = new StringBuilder();
            WriteMarshalAs(sb, level, list, wrap);
            lw.Write(sb.ToString());
        }

        void WriteMarshalAs(StringBuilder sb, int level, IList<CustomAttributeData> list, bool wrap)
        {
            foreach (var ca in list)
            {
                if (ca.Constructor.DeclaringType == typeofSystemRuntimeInteropServicesMarshalAsAttribute)
                {
                    sb.Append(" marshal(");
                    object arraySubType = null;
                    object sizeConst = null;
                    object sizeParamIndex = null;
                    object safeArraySubType = null;
                    object safeArrayUserDefinedSubType = null;
                    object iidparam = null;
                    object marshalType = null;
                    object marshalCookie = null;
                    foreach (var arg in ca.NamedArguments)
                    {
                        switch (arg.MemberInfo.Name)
                        {
                            case "MarshalType":
                                marshalType = arg.TypedValue.Value;
                                break;
                            case "MarshalCookie":
                                marshalCookie = arg.TypedValue.Value;
                                break;
                            case "ArraySubType":
                                arraySubType = arg.TypedValue.Value;
                                break;
                            case "SizeConst":
                                sizeConst = arg.TypedValue.Value;
                                break;
                            case "SizeParamIndex":
                                sizeParamIndex = arg.TypedValue.Value;
                                break;
                            case "SafeArraySubType":
                                safeArraySubType = arg.TypedValue.Value;
                                break;
                            case "SafeArrayUserDefinedSubType":
                                safeArrayUserDefinedSubType = arg.TypedValue.Value;
                                break;
                            case "IidParameterIndex":
                                iidparam = arg.TypedValue.Value;
                                break;
                        }
                    }
                    var unmanagedType = (System.Runtime.InteropServices.UnmanagedType)ca.ConstructorArguments[0].Value;
                    if (arraySubType != null && unmanagedType != System.Runtime.InteropServices.UnmanagedType.ByValArray)
                    {
                        sb.Append(UnmanagedTypeToString((System.Runtime.InteropServices.UnmanagedType)arraySubType));
                        switch ((System.Runtime.InteropServices.UnmanagedType)arraySubType)
                        {
                            ...
                            case System.Runtime.InteropServices.UnmanagedType.Interface:
                                ...
                                {
                                    sb.AppendFormat("(iidparam = {0}) ", sizeParamIndex);
                                    sizeParamIndex = null;
                                }
                                break;
                        }
                    }
                    if (safeArraySubType != null && unmanagedType == System.Runtime.InteropServices.UnmanagedType.SafeArray)
                    {
                        sb.Append(VarEnumToString((System.Runtime.InteropServices.VarEnum)safeArraySubType));
                    }
                    else if (unmanagedType == System.Runtime.InteropServices.UnmanagedType.LPArray
                        && (sizeConst != null || sizeParamIndex != null))
                    {
                        // don't write [] it's implied by the following sizeConst or sizeParamIndex
                    }
                    else
                    {
                        sb.Append(UnmanagedTypeToString(unmanagedType));
                        if (iidparam == null
                            && (unmanagedType == System.Runtime.InteropServices.UnmanagedType.Interface
                            || unmanagedType == System.Runtime.InteropServices.UnmanagedType.IUnknown))
                        {
                            sb.Append(" ");
                        }
                    }
                    if (sizeConst != null)
                    {
                        if (unmanagedType != System.Runtime.InteropServices.UnmanagedType.LPArray)
                        {
                            sb.Append(" ");
                        }
                        if (sizeParamIndex != null)
                        {
                            sb.AppendFormat("[{0} + {1}]", sizeConst, sizeParamIndex);
                        }
                        else
                        {
                            sb.AppendFormat("[{0}]", sizeConst);
                        }
                    }
                    else if (sizeParamIndex != null)
                    {
                        sb.AppendFormat("[ + {0}]", sizeParamIndex);
                    }
                    if (safeArrayUserDefinedSubType != null)
                    {
                        sb.AppendFormat(", \"{0}\"", safeArrayUserDefinedSubType);
                    }
                    if (iidparam != null)
                    {
                        sb.AppendFormat("(iidparam = {0}) ", iidparam);
                    }
                    if (marshalType != null || marshalCookie != null)
                    {
                        sb.AppendFormat("(\"{0}\",", marshalType);
                        if (wrap)
                        {
                            sb.AppendLine();
                            sb.Append(' ', level);
                        }
                        sb.AppendFormat("\"{0}\")", marshalCookie);
                    }
                    if (arraySubType != null && unmanagedType == System.Runtime.InteropServices.UnmanagedType.ByValArray)
                    {
                        sb.Append(UnmanagedTypeToString((System.Runtime.InteropServices.UnmanagedType)arraySubType));
        ...
        static string UnmanagedTypeToString(System.Runtime.InteropServices.UnmanagedType unmanagedType)
        {
        ...
        string QuoteIdentifier(string str, bool quote = false)
        {
            ...
            {
                Escape(ref str, ref quote, "\u00AE", "r");
                Escape(ref str, ref quote, "\u00A9", "c");
            }
        ...
        void WriteField(LineWriter lw, FieldInfo field)
        {
            int level = lw.Column;
            lw.Write(".field ");
            if (field.DeclaringType != null && field.DeclaringType.IsExplicitLayout)
            {
                foreach (var ca in field.__GetCustomAttributes(null, false))
                {
                    if (ca.Constructor.DeclaringType == typeofSystemRuntimeInteropServicesFieldOffsetAttribute)
                    {
                        lw.Write("[{0}] ", ca.ConstructorArguments[0].Value);
                        break;
                    }
            ...
            {
                WriteMarshalAs(lw, level, field.__GetCustomAttributes(null, false), false);
            }
            ...
            if ((field.Attributes & FieldAttributes.HasFieldRVA) != 0)
            {
                ...
                {
                    lw.Write(" at {0}_{1:X8}", (characteristics & IMAGE_SCN_CNT_CODE) != 0 ? "I" : "D", field.__FieldRVA);
                }
                else
                {
            ...
            lw.WriteLine();
            foreach (var ca in FilterPseudo(field.__GetCustomAttributes(null, false)))
            {
                lw.GoToColumn(level);
                WriteCustomAttribute(lw, ca);
            }
        ...
        void WriteModuleManifest(LineWriter lw)
        {
            ...
            for (int i = 0; i < referencedAssemblies.Length; i++)
            {
                ...
                lw.Write(".assembly extern {0}{1}",
                    ((int)asm.Flags & 512) != 0 && (compat == CompatLevel.None || compat >= CompatLevel.V45) ? "windowsruntime " : "",
        ...
        void WriteAssemblyManifest(LineWriter lw)
        {
            ...
            lw.WriteLine("{");
            foreach (var ca in assembly.__GetCustomAttributes(null, false))
            {
        ...
        void WriteModuleHeader(LineWriter lw)
        {
            ...
            foreach (var ca in module.__GetCustomAttributes(null, false))
            {
                WriteCustomAttribute(lw, ca);
            }
        ...
        void WriteAssemblyRef(LineWriter lw, Assembly asm)
        {
            if (asm != assembly)
            {
                lw.Write("[{0}]", QuoteIdentifier(referencedAssemblies[asm]));
            }
        }
        ...
        void WriteSignatureType(LineWriter lw, Type type, TypeLocation loc, bool skipGenArgs)
        {
            ...
            {
                WriteStandAloneMethodSig(lw, type.__MethodSignature, true, loc == TypeLocation.Local || loc == TypeLocation.Parameter);
            }
            ...
            else if (type.IsGenericParameter)
            {
                ...
                {
                    lw.Write("!!{0}",
                        loc == TypeLocation.MemberRef || loc == TypeLocation.DeclaringType || loc == TypeLocation.MethodGenericParameter || type.Name == null
                ...
                {
                    lw.Write("!{0}",
                        loc == TypeLocation.MemberRef || loc == TypeLocation.GenericMethodImpl || type.Name == null
                        ? (object)type.GenericParameterPosition
                        : QuoteIdentifier(type.Name));
                }
            }
            ...
            else
            {
                ...
                WriteAssemblyRef(lw, type.Assembly);
        ...
        {
            if (!type.__IsMissing && type.IsGenericType && !type.IsGenericTypeDefinition)
            {
                // TODO do we need this?
                WriteSignatureType(lw, type, TypeLocation.MemberRef);
            }
            else if (type.IsGenericParameter)
            {
