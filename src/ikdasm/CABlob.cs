...
namespace Ildasm
{
    partial class Disassembler
    {
        bool DecodeDeclSecurity(StringBuilder sb, IList<CustomAttributeData> list, int level)
        {
            try
            {
                ...
                foreach (var sec in list)
                {
                    ...
                    first = false;
                    AppendTypeName(sb, sec.Constructor.DeclaringType, sec.Constructor.DeclaringType.AssemblyQualifiedName, compat == CompatLevel.V20 || compat == CompatLevel.V40);
                    sb.Append(" = {");
                    DecodeCABlob(sb, null, sec.__GetBlob(), 0);
                    sb.Append('}');
                }
                sb.Append('}');
                return true;
        ...
        void AppendTypeName(StringBuilder sb, Type type, string typeName, bool noself = false)
        {
        ...
        bool DecodeCABlob(StringBuilder sb, ConstructorInfo constructor, byte[] blob, int level)
        {
            if (constructor != null)
            {
                try
                {
                    ...
                    ReadNamedArguments(sb, br, named, level);
                    return true;
                }
                catch (IKVM.Reflection.BadImageFormatException) { }
            ...
            else
            {
                ...
                ReadNamedArguments(sb, br, br.ReadCompressedInt(), 0);
                return true;
            }
        }
        ...
        void ReadNamedArguments(StringBuilder sb, ByteReader br, int named, int level)
        {
            for (int i = 0; i < named; i++)
            {
                ...
                AppendCATypeName(sb, fieldOrPropertyType, typeName);
        ...
        void ReadFixedArg(StringBuilder sb, ByteReader br, Type type, bool arrayElement = false)
        {
            if (type.IsArray)
            {
                ...
                else if (length == 0 && (compat == CompatLevel.V20 || compat == CompatLevel.V40))
                {
                    throw new IKVM.Reflection.BadImageFormatException();
                }
                else
            ...
            else if (type.FullName == "System.Type" && type.Assembly.GetName().Name == "mscorlib")
            {
                ...
                {
                    AppendTypeName(sb, type1, typeName);
                }
                if (!arrayElement)
                {
                    sb.Append(')');
        ...
        void AppendCATypeName(StringBuilder sb, Type type, string typeName)
        {
            if (type.IsArray)
            {
            ...
            {
                sb.Append("enum ");
                AppendTypeName(sb, type, typeName);
            }
        }
